...
import CreateScheduledMessageService from "../ScheduledMessagesService/CreateService";
...
// Utilitário para extrair e processar comandos JSON especiais do texto da IA
function extractJsonCommands(text: string): { commands: any[], cleanText: string } {
  const regex = /#\{([\s\S]*?)\}/g;
  let match;
  const commands = [];
  let cleanText = text;
  while ((match = regex.exec(text)) !== null) {
    try {
      ...
      // Log erro de parsing mas não interrompe fluxo
      logger.error(`Erro ao parsear comando JSON IA: ${e}`);
    }
  }
  return { commands, cleanText: cleanText.trim() };
}

// Função principal para processar comandos IA após resposta do ChatGPT
export async function processIaCommands({
  iaResponse,
  ticket,
  contact,
  wbot,
  ...
  const { commands, cleanText } = extractJsonCommands(iaResponse);
  let finalText = cleanText;

  for (const command of commands) {
    if (command.queueId) {
      await UpdateTicketService({
        ticketData: { queueId: command.queueId },
        ticketId: ticket.id,
        companyId
      });
      logger.info(`[IA CMD] Ticket ${ticket.id} transferido para fila ${command.queueId}`);
    }
    if (command.userId) {
      await UpdateTicketService({
        ticketData: { userId: command.userId },
        ticketId: ticket.id,
        companyId
      });
      logger.info(`[IA CMD] Ticket ${ticket.id} atribuído ao usuário ${command.userId}`);
    }
    if (command.tagId) {
      // Adiciona tag ao ticket
      const existingTag = await TicketTag.findOne({ where: { ticketId: ticket.id, tagId: command.tagId } });
if (!existingTag) {
  await TicketTag.create({ ticketId: ticket.id, tagId: command.tagId });
}
      logger.info(`[IA CMD] Tag ${command.tagId} adicionada ao ticket ${ticket.id}`);
    }
    if (command.closeTicket) {
      ...
      logger.info(`[IA CMD] Ticket ${ticket.id} fechado por comando IA`);
    }
    if (command.scheduleMessage) {
      // scheduleMessage: { datetime: ISOString, message: string, ... }
      const now = new Date();
      let scheduleDate = now;
      try {
        if (command.scheduleMessage.datetime) {
          scheduleDate = new Date(command.scheduleMessage.datetime);
        }
      } catch (e) {
        logger.error(`[IA CMD] Data inválida para agendamento: ${command.scheduleMessage.datetime}`);
      }
      await CreateScheduledMessageService({
        data_mensagem_programada: scheduleDate,
        id_conexao: ticket.whatsappId?.toString() || "",
        intervalo: "once",
        valor_intervalo: "1",
        mensagem: command.scheduleMessage.message,
        tipo_dias_envio: "",
        mostrar_usuario_mensagem: false,
        criar_ticket: false,
        contatos: [contact.number],
        tags: [],
        companyId,
        nome: `Agendado IA ${now.toISOString()}`,
        mediaPath: command.scheduleMessage.mediaPath || "",
        mediaName: command.scheduleMessage.mediaName || "",
        tipo_arquivo: command.scheduleMessage.tipo_arquivo || "",
        usuario_envio: userId ? userId.toString() : "IA",
        enviar_quantas_vezes: "1"
      });
      logger.info(`[IA CMD] Mensagem agendada para ${scheduleDate.toISOString()} no ticket ${ticket.id}`);
    }
  }
...
const checkTemporaryAI = async (
  ...
  try {
    ...
    return false;
          ...
          await verifyMediaMessage(sendMessage!, ticket, contact, null, false, false, wbot);
...
if (!msg.key.fromMe && ticket.useIntegration && ticket.status !== "open") {
  ...
  if (isAIMode && dataWebhook?.settings) {
    ...
    if (
      ...
      await handleOpenAi(settings, msg, wbot, ticket, contact, mediaSent);
    }
